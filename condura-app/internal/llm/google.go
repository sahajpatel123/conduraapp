...
func (g *Google) Chat(ctx context.Context, req ChatRequest) (ChatResponse, error) {
	...
	if req.Stream {
		...
		defer cancel()
		// Drain the stream and accumulate the text deltas. Gemini's SSE
		// format emits one Delta per chunk; each individual Delta is
		// only the latest fragment, so we must concatenate them all to
		// recover the full assistant message.
...
// in-flight request.
//
//nolint:gocognit,gocyclo // the JSON decoder-driven state machine with cancel is inherently branchy
